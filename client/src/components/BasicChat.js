...
const BasicChat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // First check if we need to process a source
      if (!sourceProvided && !sessionId) {
        // Handle source processing
        if (sourceType === 'file' && sourceFile) {
          const formData = new FormData();
          formData.append('file', sourceFile);
          
          const uploadResponse = await apiRequest(ENDPOINTS.SOURCES.UPLOAD, {
            method: 'POST',
            body: formData,
            headers: {
          ...
          const aiResponse = {
            id: Date.now() + 1,
            type: 'ai',
            content: 'Great! I\'ve processed your document. You can now ask me questions about it.',
            timestamp: new Date()
          };
          ...
          const response = await apiRequest(ENDPOINTS.SOURCES.URL, {
            method: 'POST',
          ...
          const aiResponse = {
            id: Date.now() + 1,
            type: 'ai',
            content: `I've processed the content from ${sourceLink}. What would you like to know?`,
            timestamp: new Date()
          };
          setMessages(prev => [...prev, aiResponse]);
          setIsLoading(false);
          return;
        } else if (sourceType === 'text' && sourceText) {
          // For text sources, we'll just use it as context in the first message
          setSourceProvided(true);
          
          const aiResponse = {
            id: Date.now() + 1,
            type: 'ai',
            content: 'I\'ve noted your text. What would you like to know about it?',
            timestamp: new Date()
          };
          setMessages(prev => [...prev, aiResponse]);
          setIsLoading(false);
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <label htmlFor="file-source" className="flex-1">
              <div className={`p-4 border-2 border-dashed rounded-md cursor-pointer ${sourceType === 'file' ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}`}>
                <div className="flex flex-col items-center justify-center space-y-2">
                  <UploadCloud className={`w-6 h-6 ${sourceType === 'file' ? 'text-blue-500' : 'text-gray-400'}`} />
            ...
            <div className="flex-1">
              ...
              <div className="flex rounded-md shadow-sm">
                ...
                <input
                  type="url"
                  id="link-input"
                  className={`flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-r-md border ${sourceType === 'link' ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-300'} focus:ring-blue-500 focus:border-blue-500 sm:text-sm`}
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <textarea
                id="text-input"
                rows={4}
                className={`shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border ${sourceType === 'text' ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-300'} rounded-md`}
          ...
          <div className="pt-2">
            <button
              ...
              disabled={isProcessingSource || !sourceType || (sourceType === 'file' && !sourceFile) || (sourceType === 'link' && !sourceLink.trim()) || (sourceType === 'text' && !sourceText.trim())}
              className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${isProcessingSource || !sourceType || (sourceType === 'file' && !sourceFile) || (sourceType === 'link' && !sourceLink.trim()) || (sourceType === 'text' && !sourceText.trim()) ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'}`}
            >
              {isProcessingSource ? (
                <>
